...
function saveProjectSectionId() {
  ...
  addResponsible(taskProjectId, document);
}

function addResponsible(projectId, _document) {
/*<select class="form-group" name="responsible" id="responsibleUser">*/
  ...
  callAjax('POST', url, headerConfig, reqObj, function (data) {
    ...
    for (var i=0; i<data.data.lentgh; i++) {
      ...
      option.value = data.data.user_id;
      option.innerText = data.data.firstName +' '+data.data.lastName;
